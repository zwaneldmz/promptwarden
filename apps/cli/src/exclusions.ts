/**
 * `promptwarden emit-exclusions` — renders vendor-specific, PATH-based
 ...
 * closed process and never crosses anything promptwarden can observe. The
...
import { Action, DetectorId, Policy } from "@promptwarden/policy-engine";
...
const FORMAT_SPECS: Record<ExclusionFormat, FormatSpec> = {
  ...
  "copilot-yaml": {
    ...
    caveat: [
      ...
      "its own; promptwarden makes no network calls and cannot push it to GitHub for",
      "you.",
    ],
    listItem: (p) => `- "${p}"`,
    emptyNote: "# (no active detector in the resolved policy maps to a known file pattern)",
...
export function renderExclusions(format: ExclusionFormat, policy: Policy): string {
  ...
  const lines: string[] = [
    `${c} Generated by promptwarden emit-exclusions from policy "${policy.name}".`,
    `${c} Regenerate, do not hand-edit: promptwarden emit-exclusions --format ${format}`,
    `${c}`,
    `${c} ENFORCEMENT IS VENDOR-SIDE AND BEST-EFFORT ONLY. This file does not scan or`,
    `${c} block anything itself — promptwarden's scanning runs entirely elsewhere (see`,
    `${c} \`promptwarden scan\`, the Claude Code hook, the MCP gateway). It only tells a`,
    `${c} supporting IDE feature which paths to leave out of its own context, and`,
    `${c} promptwarden cannot verify that any editor actually honors it.`,
    `${c}`,
...
const USAGE = `Usage: promptwarden emit-exclusions --format <cursorignore|copilot-yaml|aiignore> [--out <path>]
...
\`promptwarden scan\`). These are vendor-side, best-effort signals for surfaces this tool cannot
...
function parseArgs(argv: string[]): EmitExclusionsArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--format") {
      const value = argv[++i];
      if (value === undefined) {
        args.error = "promptwarden emit-exclusions: --format requires a value";
        return args;
      }
      if (!isExclusionFormat(value)) {
        args.error = `promptwarden emit-exclusions: unknown format "${value}" (expected cursorignore, copilot-yaml, or aiignore)`;
        return args;
      }
      args.format = value;
    } else if (a === "--out") {
      const value = argv[++i];
      if (value === undefined) {
        args.error = "promptwarden emit-exclusions: --out requires a path";
        return args;
      }
      args.out = value;
    } else if (a === "--help" || a === "-h") {
      ...
      args.error = `promptwarden emit-exclusions: unknown argument "${a}"`;
      return args;
    }
  }
  if (args.format === undefined) {
    args.error = "promptwarden emit-exclusions: --format is required";
  }
  return args;
}
...
export async function runEmitExclusions(argv: string[]): Promise<number> {
  ...
  if (args.out) {
    try {
      ...
      process.stderr.write(`promptwarden emit-exclusions: could not write "${args.out}": ${(err as Error).message}\n`);
      return 3;
    }
    process.stderr.write(
      `promptwarden emit-exclusions: wrote ${args.out} (format: ${args.format}, policy: "${policy.name}", source: ${policySource})\n`,
    );
  } else {
