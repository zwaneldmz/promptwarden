# Deploying Wardkeep via Google Admin Console (Chrome)

**Note:** not yet verified against a real managed tenant; treat the steps below as unconfirmed until tested end-to-end.

Force-install Wardkeep across a fleet and push a managed policy, no end-user install
...
- The Wardkeep **Chrome Web Store extension ID**. The Chrome Web Store listing has not
  shipped yet. Until then, use `<WARDKEEP_EXTENSION_ID>` as a placeholder everywhere below
...
4. Paste `<WARDKEEP_EXTENSION_ID>` → click **Save**.
5. On the newly added row for Wardkeep, set **Installation policy** to
   **Force install**.
6. Click **Save** at the top right of the page.

Devices in that OU will install Wardkeep on their next policy fetch (typically within a
...
| Field | Type | Meaning |
|---|---|---|
| `policy` | string | A Wardkeep policy document, **JSON-encoded as a string** (see the warning below) |
...
1. Still on **Apps & extensions** → **Users & browsers**, click the Wardkeep row (not the
...
1. **On a test device already in the target OU**, open `chrome://extensions`. Confirm
   Wardkeep is listed, enabled, and that its card shows it was **installed by your
...
2. Open `chrome://policy`. Click **Reload policies** to force an immediate refetch. Below the
   main browser-policy table, look for a card for the Wardkeep extension (Chrome renders
