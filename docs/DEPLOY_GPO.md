# Deploying Wardkeep via Windows Group Policy (on-prem AD)
...
- Group Policy Management Console (GPMC) with the Chrome and/or Edge ADMX templates loaded
  ...
  provide an entry for Wardkeep's own `policy`/`extraHosts` schema — that's set via raw
...
- The Wardkeep Chrome Web Store extension ID (`<WARDKEEP_EXTENSION_ID>`) and Edge
  Add-ons extension ID (`<WARDKEEP_EDGE_EXTENSION_ID>`) — placeholders until the store
...
HKLM\SOFTWARE\Policies\Google\Chrome\ExtensionInstallForcelist
  1  (REG_SZ) = <WARDKEEP_EXTENSION_ID>;https://clients2.google.com/service/update2/crx

HKLM\SOFTWARE\Policies\Microsoft\Edge\ExtensionInstallForcelist
  1  (REG_SZ) = <WARDKEEP_EDGE_EXTENSION_ID>;https://edge.microsoft.com/extensionwebstorebase/v1/crx
```
...
```
HKLM\SOFTWARE\Policies\Google\Chrome\3rdparty\extensions\<WARDKEEP_EXTENSION_ID>\policy
...
HKLM\SOFTWARE\Policies\Google\Chrome\3rdparty\extensions\<WARDKEEP_EXTENSION_ID>\policy\extraHosts
  1  (REG_SZ) = https://internal-chat.example.com/*
  2  (REG_SZ) = https://llm.example.org/*
```

For Edge, mirror the same two keys under
`HKLM\SOFTWARE\Policies\Microsoft\Edge\3rdparty\extensions\<WARDKEEP_EDGE_EXTENSION_ID>\policy`.
...
- **Declaring `extraHosts` here does not yet make Wardkeep scan those hosts.** The
...
1. On a domain-joined test machine already receiving the GPO, run `gpupdate /force`, then
   ...
   reg query "HKLM\SOFTWARE\Policies\Google\Chrome\3rdparty\extensions\<WARDKEEP_EXTENSION_ID>\policy\extraHosts"
   ```
2. Open `chrome://extensions` (or `edge://extensions`). Confirm Wardkeep appears,
...
3. Open `chrome://policy` (or `edge://policy`) → **Reload policies**. Confirm the
   Wardkeep extension card shows the pushed `policy`/`extraHosts` values with no schema
