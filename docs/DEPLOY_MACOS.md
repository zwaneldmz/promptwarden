# Deploying Wardkeep on macOS (Chrome and Edge)
...
- The Wardkeep **Chrome Web Store extension ID** (`<WARDKEEP_EXTENSION_ID>`) and, separately, the **Edge Add-ons extension ID** (`<WARDKEEP_EDGE_EXTENSION_ID>`) — placeholders until either store listing ships, same caveat as `DEPLOY_GOOGLE_ADMIN.md`/`DEPLOY_INTUNE.md`/`DEPLOY_GPO.md`. If you're deploying a self-hosted CRX instead (`DEPLOY_SELF_HOSTED_CRX.md`), use that build's own extension ID here, not the (unissued) store ID — see that doc's warning section for why mixing them breaks policy delivery silently.
...
1. **The managed-preference domain names**, which replace the Windows registry paths:
   ...
   | Wardkeep's own policy (Chrome) | `...\3rdparty\extensions\<id>\policy\{policy,extraHosts}` | `com.google.Chrome.extensions.<WARDKEEP_EXTENSION_ID>`, keys `policy` and `extraHosts` |
   | Wardkeep's own policy (Edge) | `...\3rdparty\extensions\<id>\policy\{policy,extraHosts}` | `com.microsoft.Edge.extensions.<WARDKEEP_EDGE_EXTENSION_ID>`, keys `policy` and `extraHosts` |
...
The `.mobileconfig` payload below force-installs Wardkeep for Chrome. Duplicate the same payload with `com.microsoft.Edge` and the Edge extension ID/update URL for Edge; you can ship both in one profile as two separate `PayloadContent` entries.
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>tech.abantu.wardkeep.chrome-forcelist</string>
      <key>PayloadUUID</key>
      <string>REPLACE-WITH-A-FRESH-UUID</string>
      <key>PayloadVersion</key>
      <integer>1</integer>
      <key>PayloadDisplayName</key>
      <string>Wardkeep — Chrome force-install</string>
      <key>PayloadContent</key>
      <dict>
          ...
          <array>
            <dict>
              ...
              <dict>
                ...
                <array>
                  <string>&lt;WARDKEEP_EXTENSION_ID&gt;;https://clients2.google.com/service/update2/crx</string>
                </array>
              </dict>
            </dict>
          </array>
  ...
  <key>PayloadDisplayName</key>
  <string>Wardkeep macOS policy</string>
  <key>PayloadIdentifier</key>
  <string>tech.abantu.wardkeep.macos-policy</string>
...
<dict>
  ...
  <string>com.apple.ManagedClient.preferences</string>
  <key>PayloadIdentifier</key>
  <string>tech.abantu.wardkeep.chrome-extension-policy</string>
  <key>PayloadUUID</key>
  <string>REPLACE-WITH-A-FRESH-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
  <key>PayloadDisplayName</key>
  <string>Wardkeep — Chrome extension policy</string>
  <key>PayloadContent</key>
  <dict>
    <key>com.google.Chrome.extensions.&lt;WARDKEEP_EXTENSION_ID&gt;</key>
    <dict>
      <key>Forced</key>
      <array>
        <dict>
...
Note the domain name (`com.google.Chrome.extensions.<WARDKEEP_EXTENSION_ID>`) is literally `<extension-id>` appended to the browser's own domain with a `.` — the extension ID is part of the *domain name itself*, not a key inside it, matching how the Windows registry path embeds the ID as a path segment rather than a value.
...
Edge: duplicate this whole payload with `com.microsoft.Edge.extensions.<WARDKEEP_EDGE_EXTENSION_ID>` and the Edge extension ID.
...
Intune's **Settings Catalog** for macOS mirrors its Windows counterpart's gap: it has built-in entries for a handful of well-known Chrome/Edge policies, but nothing for Wardkeep's own per-extension schema. Two options, same shape as `DEPLOY_INTUNE.md`'s Windows guidance:

### Option A — Settings Catalog, if your tenant's catalog happens to list it

**Devices** → **macOS** → **Configuration** → **Create** → **Settings catalog**, search for **Chrome** / **Edge** → **Extension install forcelist**. Confirm your tenant's catalog actually surfaces this before relying on it — catalog contents are Microsoft/Google-maintained and can change. This only covers Step 1 (force-install); the catalog has no entry for Wardkeep's own `policy`/`extraHosts` schema, so Step 2 always needs Option B below regardless.
...
# Step 2 — Wardkeep's own policy (Chrome)
sudo defaults write "/Library/Managed Preferences/com.google.Chrome.extensions.<WARDKEEP_EXTENSION_ID>" \
