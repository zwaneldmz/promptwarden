...
server-side egress. The funnel runs on one paid asset: a 7-day **Shadow-AI Exposure Check**
(event-mode logging, local export, founder-compiled 1-page report), sold from W3 at
€1,500–2,500 per engagement (MSP-resellable at ~35% margin), that quantifies the prospect's
own pain in their own numbers. The free tier is the self-install extension only — no console,
no ingest backend, no AI tier until a signature forces them.
...
posture and deletes plan 2's W5 ingest/Neon/Worker week. The export is a
**k-anonymous aggregate** (counts per detector/host, MIN_COHORT 5, no raw text, no
per-user record) — a raw per-employee export would make the vendor an Art. 28 GDPR
processor over employee data, the exact liability the zero-processing posture exists to
avoid. Console Lite exists only behind a
...
|---|---|---|---|
| A | Outbound & pipeline (MSP + direct-org); owns the prospect list (400+ names, weekly replenishment) | **Founder only** | ~18h from W3 |
...
|---|---|---|---|
| **W1** | 07-27→08-02 | Kill bar committed. Apache-2.0 `LICENSE`+`NOTICE`+`TRADEMARKS.md`, `SECURITY.md`, DCO. Fix false doc claims (no CI exists; "14 tests" → 20). Fix 3 correctness holes: click-path `activeEditable(null)` no-op (track last-focused editable via `focusin`), stale `bypassNextSubmit` gate (expire on timer + input), missing `submit` listener — each verified by the scripted live-browser regression (`tools/e2e-smoke.mjs`; the extension has no DOM test harness by design) (C). Real CI: tsc → tests → bench gate → **no-egress gate** (fails on any network API outside dist/). Delete `policyUrl` from managed_schema. Icons 16–128. **Repo public.** **Council engineering exception — done 2026-07-25, ahead of the W2/W1 schedule below**: click-path fix; hardened CI gates (tsc/tests/bench/no-egress, now also scanning `popup.js`/`popup.html`, also checking `new Image()`/`connectNative`); and the **k-anonymous aggregate event-export** (`tools/pwreport.mjs` + popup "Export aggregate report" button — day-bucketed counts only, MIN_COHORT 5 cohort suppression, no raw text, no per-user record, tests passing) — this is the W2 Exposure Check pipeline, pulled forward. Extensible-host plumbing is *partially* shipped: `optional_host_permissions` + managed-schema `extraHosts` field are done; the background-side dynamic registration that actually scans an admin-added host is **spec only, not yet wired** — see `docs/HOST_COVERAGE.md`, which is explicit that every exposure number this repo can currently produce is a 7-default-host number. Edge Add-ons account + submit **unlisted** (locks extension ID, starts the longest clock — Edge first, the actual DACH Intune fleet). Chrome Web Store account created; submission held until W2, once the privacy-policy URL exists. Sending domain + SPF/DKIM/DMARC warmup. **Book the Austrian Arbeitsrecht+Datenschutz lawyer for W5 now** — AT firms close in August. Warm-network sweep: 20 intro paths, 8 asks. | Public repo, green CI, threat model |
| **W2** | 08-03→08-09 | `docs/THREAT_MODEL.md` (what we honestly don't stop). **Exposure Check pipeline (event export + `tools/pwreport`) shipped early — see W1 row**; W2 work here is packaging it into the 1-page HTML/PDF flow a prospect receives, not building the aggregation itself. Landing page (zero cookies, self-hosted fonts, Impressum, `/trust` page — privacy-policy URL now live). Chrome Web Store submitted **unlisted**, now that the privacy-policy URL exists. 3-min demo video. 40 MSP + 40 direct-org named prospects (WKO, MS Partner directory, RMM partner finders; 5–50-employee MSPs) — first tranche toward the 400+ name list, weekly replenishment owned by Workstream A. Signed release v0.2.0 (cosign keyless + provenance). **Gate G1.** | Exposure Check report from own devices |
| **W3** | 08-10→08-16 | **Shadow-AI Exposure Check goes on sale**: €1,500–2,500 per engagement, MSP-resellable at ~35% margin (free tier = self-install extension only). Cold sequence live (12 MSPs/wk, 6 touches, German, AI-Act hook — verify the 08-02 applicability scope before using it in copy). Force-install docs: Google Admin + Intune, copy-pasteable, **verified on a real managed profile** — incl. self-hosted CRX fallback so pilots never depend on store review. Weekly Playwright smoke across 7 hosts (local CI, no egress). Popup "Problem melden" → prefilled GitHub issue. Detector precision: BIN gating on credit_card, false-positive corpus test (invoice/order/tracking numbers must not fire), `bulk_pii` detector (N+ distinct PII in one payload — maps to "someone pasted our customer list"). | "50 seats in 10 minutes" screen recording |
| **W4** | 08-17→08-23 | **Options page = local-first policy builder** (rules table, live preview on the real engine, import/export, "Copy for Google Admin" button; read-only under managed policy) — same code shipped as public **/playground** URL (zero-install cold-email asset). Edge Add-ons live-status check (submitted W1; confirm live or escalate per G2). Trust pack core: TP-01 data-flow, TP-08a zero-processing statement, DSFA threshold + template, ROPA block, TOM. Pilot profiles: observe/warn/enforce JSON. **Gate G2.** | Playground URL |
| **W5** | 08-24→08-30 | **Lawyer review** (booked W1, 5h — covers ~1–2 instruments, scoped to the **AT Betriebsvereinbarung + pilot agreement only**): Betriebsvereinbarung AT (§96a ArbVG is the governing test for event-level employee logging — BV required but arbitrable; most target-ICP orgs have no Betriebsrat, so the governing instrument there is a **§10 AVRAG individual agreement** instead: event-only, no content, no individual attribution, MIN_COHORT 5, no-disciplinary-use), pilot agreement. DSFA and employee Art. 13 one-pager drafted from template, lawyer-reviewed only if the 5h slot has room. **DE variant (BetrVG §87(1) Nr. 6) deferred until a DE prospect exists** — separate German counsel, unbudgeted. German UI strings (`chrome.i18n`, de-AT). `retentionDays` age-based pruning shipped early with the council fixes (2026-07-25) — W5 scope is only documenting it as a DPIA line item + user-clear in standalone. `PRIVACY_DATA_SHEET.md` **generated from the `toLogRecord` field list with a test asserting doc matches code**. Book September hard. | Lawyer-reviewed trust pack v1.0 |
| **W6** | 08-31→09-06 | Trust pack summary live at `/trust` (DE+EN) — one-pager + request form only; the DPIA, BV template, and data sheet are **gated**, sent per-prospect after a discovery call, not published for anyone to download *(a free public DPIA/BV template hands prospects' works councils and competitors negotiating leverage for nothing — gating turns the document into a qualified-lead signal instead of a giveaway)*. Show HN + r/msp methodology post (1 day, credibility not pilots). Pitch 2 speaking slots (Steuerberater/UBIT events). First Exposure Checks at friendly orgs. 2 pilot proposals out. **Gate G3.** | Live trust page + install count |
...
| G2 | 08-23 | 8 discovery calls, 3 Checks agreed, **≥10 September meetings booked**, store live or escalated, prospect list ≥150 of the 400+ target (Workstream A replenishing weekly) | Thin September calendar is the real failure — phone-first + warm-intro-only for 2 weeks |
| G3 | 09-06 | 20 cumulative calls, 5 Checks in flight, 150 installs or 25 stars, 2 proposals, prospect list ≥250 of the 400+ target (20 cumulative calls only pencils out through W14 against a continuously replenished list, not the static 80-name W1 seed) | Change ICP (MSP↔direct, AT↔DE), not the product |
...
2. **Mon PM — start the two uncontrollable clocks.** Edge Add-ons: account, icons, DE+EN copy, permission justification, upload **unlisted** (Edge first — Chrome Web Store account created now too, submission held for W2 once the privacy-policy URL exists). Sending domain live, SPF/DKIM/DMARC, warmup started. ~4h.
...
| Risk | Mitigation |
|---|---|
| Store review stalls past W6 | Unlisted upload W1 (Edge — ID assigned immediately), Chrome parallel from W2 once the privacy-policy URL exists, self-hosted CRX force-install verified W3 — store off the pilot critical path |
...
| Works council blocks deployment | §10 AVRAG individual-agreement default for the (Betriebsrat-less) target ICP, §96a ArbVG BV path ready for the minority that have one, lawyer-reviewed BV template handed over at call #1, target low-density sectors (Steuerberater, Lohnverrechnung, Personaldienstleister, Makler) |
...
**Budget:** lawyer €1,200–2,500 (5h — AT Betriebsvereinbarung + pilot agreement only, ~1–2 instruments; DE variant deferred/unbudgeted, separate German counsel) · store fees ~$25 · domain/email €80 · it-sa ticket+hotel €600 · Sales Navigator €300 · code-review letter ~€2,000 (W6, optional until an MSP asks) ≈ **€4,500 ceiling**.
